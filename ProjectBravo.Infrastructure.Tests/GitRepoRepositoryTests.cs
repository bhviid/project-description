...
public class GitRepoRepositoryTests
{
    ...
    public async Task CreateAsync_given_newer_but_already_existing_repo_updates_existing()
    {
        ...
        _context.Repos.Add(new GitRepository
        {
            ...
            Commits = new HashSet<Commit> { new Commit { Message = "HVASÅ", Date = DateTime.Today.AddDays(-10), Author = new Author { Name = "asger" } } },
            LatestCommitDate = DateTime.Now,
        });
        _context.SaveChanges();

        var toAdd = new GitRepositryCreateDTO("Etrepo2",
            new List<string>() { "asger", "Bøllebob" },
            new List<CommitCreateDTO>()
            {
                new CommitCreateDTO(DateTime.Today, "This is the newest commit!","Bøllebob", "Etrepo")
            });
        ...
        // Assert
        gRepo.Authors.Should().BeEquivalentTo(new[] { "asger", "Bøllebob" });
    ...
    public async Task Update_given_existing_succeeds()
    {
        ...
        _context.Repos.Add(new GitRepository
        {
            Name = "Etrepo",
            Authors = new HashSet<Author> { new Author { Name = "Frederik" } },
            Commits = new HashSet<Commit> { new Commit { Message = "HVASÅ", Date = DateTime.Now.AddHours(-5), Author = new Author { Name = "asger" } } },
            LatestCommitDate = DateTime.Now,
        });
